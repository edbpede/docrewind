...
- [x] License hygiene: confirm `LICENSE` (AGPL-3.0-or-later) and per-file SPDX headers across first-party source; add a CI license-audit step rejecting AGPL-incompatible dependencies (PRD §11.6). *(All 93 TS/TSX + 6 shell scripts carry SPDX; `scripts/license-audit.ts` + `audit:licenses` wired into `ci.yml` and `release.yml`.)*
