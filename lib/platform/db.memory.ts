...
// lib/platform/db.ts exactly (parser-version invalidation, LRU-drops-raw-first), so one
 ...
 * after a `get` cannot reach back into the store (header parity with lib/platform/db.ts).
 */
