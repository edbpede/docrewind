...
describe("PrivacyBanner (collapsible disclosure)", () => {
  ...
  it("toggles the detail open/collapsed and announces state via aria", async () => {
  ...
  it("renders the approximation note only when supplied", () => {
    ...
    const withNote = render(PrivacyBanner, {
      props: { approximationNote: strings.privacy.approximationNote },
    });
